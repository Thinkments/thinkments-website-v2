          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {events.length === 0 ? (
                ...
                events.map((evt) => (
                  <tr key={evt.id} className="border-b border-white/5 hover:bg-[#0f172a]/40 backdrop-blur-xl/5 transition-colors">
                    <td className="p-4 text-white whitespace-nowrap">{evt.date}</td>
                    <td className="p-4 font-medium text-white">{evt.title}</td>
                    <td className="p-4">
                      <span className="px-2 py-1 bg-[#0f172a]/40 backdrop-blur-xl/10 text-white/70 rounded text-xs uppercase tracking-wider">
                        {evt.type}
                      </span>
                    </td>
                    <td className="p-4">
                      <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${
                        evt.status === 'published' 
                          ? 'bg-emerald-500/20 text-whitemerald-400'
                          : 'bg-indigo-900/200/20 text-blue-400'
                      }`}>
