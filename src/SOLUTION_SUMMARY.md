...
Since we **cannot** change the Content-Type header (fundamental limitation), we provide multiple workarounds:
...
We've implemented the **optimal solution** for serving robots.txt and sitemap.xml within Figma Make's infrastructure constraints. The solution is:
...
The Google Search Console "HTML" warning is expected and has **zero impact** on SEO performance. Google will index all pages normally through the multiple discovery methods we've implemented.
